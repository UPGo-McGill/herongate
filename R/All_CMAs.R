CT_raw_all <- get_tongfen_ca_census(
  regions = list(C = "01"), 
...
CT_all <- 
  ...
  left_join(CT_geom_all, by = "TongfenUID") |> 
...
ph_all <- st_point(c(-79.38364973603952, 43.65276898078397)) |> 
  st_sfc(crs = 4326) |> 
  st_transform(32617)
...
ph_dist_all <- 
  ...
  mutate(ph_dist = as.numeric(st_distance(st_transform(geometry, 32617), ph_all))) |> 
  st_drop_geometry()

CT_final_all <- 
  CT_final_all |> 
  left_join(ph_dist_all, by = "census_tract")
