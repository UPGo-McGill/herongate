CT_raw_TO <- get_tongfen_ca_census(
...
CT_TO <- 
  ...
  left_join(CT_geom_TO, by = "TongfenUID") |> 
...
ph_TO <- st_point(c(-79.38364973603952, 43.65276898078397)) |> 
  st_sfc(crs = 4326) |> 
  st_transform(32617)
...
ph_dist_TO <- 
  ...
  mutate(ph_dist = as.numeric(st_distance(st_transform(geometry, 32617), ph_TO))) |> 
  st_drop_geometry()

CT_final_TO <- 
  CT_final_TO |> 
  left_join(ph_dist_TO, by = "census_tract")
