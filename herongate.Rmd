...
Now we start to clean up the dataset, first by pivoting the table by year and then by sorting out the variables that were handled differently in the different census years. We also update `CT_geom` and remove the geometry from the main `CT` dataset.
...
model_value_change_fe <- 
  lm(value_change_pct ~ value_2006 + black_ratio_2016 + black_ratio_change + 
       ...
       p_college_2016 + ph_dist + city - 1, data = CT_final)
